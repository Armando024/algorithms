...
object_t *delete2(tree_node_t *tree, key_t delete_key)
...
int find_next_larger(tree_node_t *tree,int q ) {
    if (tree->left==NULL) { //if tree is empty
        return -1;}
    else {
        tree_node_t* temp=tree;
        key_t max;//I'm going to look for maximum
        while(temp->right!=NULL) {
            temp=temp->right; }
        max=temp->key;
        if (q>max) { return q;} // if there is no leaf larger that q it returns q
        temp=tree;
        parent_ptr head=new Parent;
        head->back=NULL;
        head->current=tree;
        while(true) {
            if(temp->right==NULL) {
                if(temp->key>q) {
                    ...
                    head=head->back;
                    while( (temp->key)==(head->current->right->key) ) {
                        temp=head->current;
                        head=head->back;
                    }
                    temp=head->current->right;
                    if ( temp->key>q && temp->right==NULL) { 
                        return temp->key;
                    }}}
            if(q<temp->key) {
                temp=temp->left;
               parent_ptr tail=new Parent;
                tail->back=head;
                tail->current=temp;
                head=tail;
            }else {
...
{  tree_node_t *searchtree;
    ...
    { if( nextop == 'i' )
        ...
        { int delkey, *delobj;
            scanf(" %d", &delkey);
            delobj = delete2( searchtree, delkey);
            if( delobj == NULL )
        ...
        if (nextop=='a') {
            int key2;
            scanf(" %d",&key2);
            int answer=find_next_larger(searchtree,key2);
            printf("%d\n",answer);
        }
    }
    return(0);
